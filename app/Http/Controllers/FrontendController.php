...
class FrontendController extends Controller
{
    public function index(Request $request)
    {
        ...
        $all_products = Product::where('status', 1)->with('category')->select('id', 'product_name', 'product_code', 'product_quantity', 'product_details', 'product_color','product_size','selling_price','discount_price')->orderBy('id','desc')->paginate(12);
       ...
       $hot_deals = Product::where('status', 1)->where('hot_deal', 1)->with(['brand','files'])->select('id', 'product_name', 'product_code', 'product_quantity', 'product_details', 'product_color','product_size','selling_price','discount_price')->orderBy('id','desc')->get();
